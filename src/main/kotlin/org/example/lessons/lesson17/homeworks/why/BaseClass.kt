...
abstract class BaseClass(
    ...
    // потому что в ChildrenClass создали собственное поле, а не обращались к приватному полю с таким же названием из BaseClass
    private val privateVal: String,
    // 2. объясни, почему это поле недоступно в main()
    // к protected полю можно обращаться только внутри класса и в классах наследниках
    ...
    // возвращает protected тип ProtectedClass, если сделаем публичной, раскроем реализацию ProtectedClass
    protected open fun getProtectedClass() = ProtectedClass()
    protected open fun verifyPublicField(value: String): Boolean {
        return value.length < 3
    }
    // 8. Распечатай getAll() и объясни, почему в поле "generate" другой текст
    // в дочернем классе переопределили метод и вызываем переопределенный вариант
    ...
    // сам класс объявлен private, метод не может быть public/protected
