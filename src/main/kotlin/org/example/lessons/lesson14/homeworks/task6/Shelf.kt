...
class Shelf (val capacity: Int, val items: MutableList<String>) {
    fun addItem (nameOfItem: String): Boolean {
        val currentCapacity = items.sumOf { it.length }
        return if (currentCapacity + nameOfItem.length <= capacity) {
    ...
    fun removeItem (nameOfItem: String): Boolean {
        return if (nameOfItem in items) {
            items.remove(nameOfItem)
            true
        } else {
            false
        }
    ...
    fun canAccommodate(nameOfItem: String): Boolean {
        val currentCapacity = items.sumOf { it.length }
        return currentCapacity + nameOfItem.length <= capacity
    }

    fun containsItem (nameOfItem: String): Boolean {
